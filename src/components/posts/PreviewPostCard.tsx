...
import { PostPollOption } from "@/actions/posts";
...
export default function PreviewPostCard({ post, communitySlug }: Props) {
    if (!post) {
        return (
            <div className="space-y-2">
                ...
                <Card className="w-full max-w-sm p-4! gap-0!">
                    <div className="flex flex-col items-center justify-center py-8 text-center gap-2">
                        <Megaphone className="size-8 text-muted-foreground/30" strokeWidth={1} />
                        <p className="text-xs text-muted">Fill in the post details to see a preview</p>
                    </div>
                </Card>
            </div>
        )
    ...
    const totalVotes = 0
    const userVotedOptionId = 0

    // Group reactions by emoji
    const reactionMap = { '😂': { count: 5, userReacted: true }, '😍': { count: 3, userReacted: false } }
                ...
                {post.type === 'POLL' && post.pollOptions.length > 0 && (
                    <div className="px-5 py-3 space-y-2">
                        {post.pollOptions.map((option, index) => {
                            ...
                            const isVoted = userVotedOptionId === index
